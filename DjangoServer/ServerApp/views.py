...
def notifs(request):
    """Endpoint to handle notifications to frontend
        Returns:
            String : Notif sent
    """
    if True: # request.remote_addr=="127.0.0.1":
...
def get_uptime(request):
    ...
    """
    if not check_auth():
        return JsonResponse("Auth error", safe=False)
    try:
...
def check_status(request):
    ...
    global processid
    if processid:
        return JsonResponse(processid, safe=False)
    else:
        return JsonResponse("There is no processID", safe=False)
...
def sleep(request):
    ...
    if request.method == 'GET':
        try:
            if not processid:
                ...
                return JsonResponse("201 from the beginning... ", safe=False)
            else:
                return JsonResponse("200 from the beginning... ", safe=False)
        except Exception as e:
            print(e)
        return JsonResponse("Something went wrong - in the beginning...", safe=False)
    ...
    try:
        if not processid:
            ...
            return JsonResponse("201 from the end...", safe=False)
        else:
            return JsonResponse("200 from the end...", safe=False)
    except Exception as e:
        print(e)
        return JsonResponse("Some error occurred", safe=False)
